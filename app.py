...
app.config['MAIL_PASSWORD'] = 'a6596ca568fb8b'
...
def register():
    ...
    # test = User.query.filter(email=email).first()
    # if test:
...
def login():
    ...
    test = User.query.filter(email=email, password=password).first()
    if test:
...
def retrieve_password(email=str):
    ...
    if user:
        msg = Message('Your Planetary API password is' + user.password, sender='admin@planetaryapi.com', recipients=[email])
        mail.send(msg)
...
def planet_details(planet_id=int):
    ...
    if planet:
        ...
        return jsonify(message='That plannet does not exist'), 404
